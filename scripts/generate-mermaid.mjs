...
async function buildCISequence() {
  ...
  for (const f of yamls.slice(0, 4)) {
    ...
    const jobNames = [];
    // Look for 'jobs:' section and extract job names
    const jobsMatch = /^jobs:\s*$/m.exec(text);
    if (jobsMatch) {
      const startIdx = jobsMatch.index + jobsMatch[0].length;
      const afterJobs = text.slice(startIdx);
      // Extract job names (2-space indented keys)
      for (const line of afterJobs.split(/\r?\n/)) {
        // Stop if we hit a non-indented key
    ...
    for (const j of jobNames.slice(0, 6)) {
      ...
      // Extract the job block: find the job key and extract until the next job or top-level key
      const jobPattern = new RegExp(`^  ${j}:\\s*$`, "m");
      const jobMatch = jobPattern.exec(text);
      if (jobMatch) {
        const startIdx = jobMatch.index + jobMatch[0].length;
        const afterJob = text.slice(startIdx);
        let jobBlock = "";
        for (const line of afterJob.split(/\r?\n/)) {
          // Stop if we hit another job (2-space indent with colon) or non-indented key
          if (/^  [A-Za-z0-9_-]+:\s*$/.test(line) || /^\S/.test(line)) break;
          jobBlock += line + "\n";
        }
        const stepNames = Array.from(jobBlock.matchAll(/^\s*-\s+name:\s*(.+)$/gm)).map(x => x[1].trim()).slice(0, 8);
